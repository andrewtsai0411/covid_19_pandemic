...
total_case = daily_report['confirmed'].sum()
...
def filter_global_map(country_names):
    ...
    for country, province, county, c, d in zip(countries, provinces, counties, confirmed, deaths):
        if county:
            marker_infomation = [(country, province, county),c,d]   
        elif province:
            marker_infomation = [(country, province),c,d]
        else:
            marker_infomation = [country,c,d]
        information_when_hover.append(marker_infomation)
...
with gr.Blocks() as global_map_tab:
    ...
    with gr.Row():
        gr.Label(f'{total_case:,}', label='Total cases')
...
demo = gr.TabbedInterface([global_map_tab, country_time_series_tab], ['Gbobal Map', 'Country Time Series']
